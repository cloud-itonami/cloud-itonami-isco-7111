(ns housebuilder.actor
  "HouseBuilderActor — the ISCO-08 7111 house-building job-site
  ...
  accountingsupport.actor (and closely on cloud-itonami-isco-9311's
  ...
  The unconditional invariant: the House Builder Advisor can never
  directly commit an operating record the HouseBuilderGovernor
  refuses, and can never make this actor finalize a
  structural-work-execution decision (e.g. a specific framing or
  foundation step) or override a site safety officer's/foreman's
  judgment — every commit-record! call is gated behind `:decide`, and
  that decision class is a permanent hard block regardless of
  disposition path. This actor coordinates JOB-SITE
  SCHEDULING/LOGISTICS ONLY — it never performs construction work or
  authorizes structural work itself."
...
(defn build-graph
  "Build a compiled HouseBuilderActor graph. `store` implements
  `housebuilder.store/Store`. `advisor` implements
  `housebuilder.advisor/Advisor` (defaults to `mock-advisor`).
  ...
  (-> (g/state-graph
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:builder-id (:builder-id request)
                                    :op (:op proposal)
